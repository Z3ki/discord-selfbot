      ...
      protocol
        .get(requestUrl, (res) => {
          ...
          res.on('end', async () => {
            ...
            if (!validation.valid) {
              logger.warn(`File type validation failed: ${validation.reason}`, {
                url,
                mimeType,
              });
              reject(
                new Error(`File type validation failed: ${validation.reason}`)
              );
              return;
            }
  ...
  return new Promise((resolve, reject) => {
    ffmpeg(videoPath)
      .outputOptions([
        '-vn', // No video
        '-acodec',
        'libmp3lame', // MP3 codec for compatibility
        '-ab',
        '128k', // Audio bitrate
        '-ar',
        '44100', // Sample rate
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        const { targetFrames, optimalFps } = calculateVideoProcessingParams(
          metadata.duration,
          metadata.size
        );

        logger.info(
          `Video: ${metadata.duration.toFixed(1)}s, ${Math.round(metadata.size / 1024 / 1024)}MB - extracting ${targetFrames} frames at ${optimalFps.toFixed(2)} fps`
        );
        ...
        ffmpeg(videoPath)
          .outputOptions([
            '-vf',
            `fps=${optimalFps}`, // Dynamic FPS based on video characteristics
            '-q:v',
            '2', // High quality JPEG
            '-frames:v',
            targetFrames.toString(), // Dynamic frame count
          ])
          .output(framePattern)
          .on('end', async () => {
            try {
              ...
              reject(
                new Error(`Failed to read extracted frames: ${error.message}`)
              );
            }
          })
          .on('error', (error) => {
            reject(
              new Error(`Failed to extract video frames: ${error.message}`)
            );
          })
          .run();
      } catch (error) {
        reject(
          new Error(
            `Failed to prepare video frame extraction: ${error.message}`
          )
        );
      }
    })();
  ...
  return new Promise((resolve, reject) => {
    ...
    ffmpeg(gifPath)
      .outputOptions([
        ...
        '2', // High quality PNG
        '-frames:v',
        '8', // Limit to maximum 8 frames for GIFs
      ])
      .output(framePattern)
      .on('end', async () => {
        try {
          ...
          const frameFiles = (await fs.promises.readdir(outputDir))
            .filter(
              (file) => file.startsWith('gif_frame_') && file.endsWith('.png')
            )
            .sort()
            .map((file) => path.join(outputDir, file));
          ...
          reject(
            new Error(`Failed to read extracted GIF frames: ${error.message}`)
          );
        }
      })
      .on('error', (error) => {
      ...
      protocol
        .get(requestUrl, (res) => {
          ...
          const tempDir = mkdtempSync(
            path.join(tmpdir(), 'discord-selfbot-media-')
          );
          ...
          fileStream.on('finish', async () => {
            ...
            try {
              ...
              const { frameFiles, actualFps, duration, targetFrames } =
                frameResult;
              ...
              await Promise.all(
                frameFiles.map((file) => fs.promises.unlink(file))
              );
        ...
        .get(requestUrl, (res) => {
          ...
          fileStream.on('finish', async () => {
            ...
            try {
              ...
              resolve({
                frames,
                fallbackText: `**ANIMATED GIF MEDIA**: ${url} - extracted ${frames.length} frames`,
              });
            } catch (error) {
              // Clean up on error
        ...
        .get(requestUrl, async (res) => {
          ...
          fileStream.on('finish', async () => {
            ...
            try {
              ...
              logger.info(
                `Whisper transcription completed: ${transcriptText.substring(0, 100)}...`
              );
...
export async function processMessageMedia(
  ...
  if (asyncProcessing) {
    ...
    if (message.attachments && message.attachments.size > 0) {
      ...
      logger.debug(
        `Processing ${attachmentsArray.length} attachments synchronously for images`
      );
      ...
      for (const attachment of attachmentsArray) {
        ...
        try {
          if (
            ...
            logger.debug(
              `Processing image attachment synchronously: ${attachment.url}`
            );
            ...
            if (imageData) {
              ...
              logger.debug(
                `Successfully processed image synchronously: ${attachment.url}`
              );
            } else {
              logger.warn(
                `Failed to download image synchronously: ${attachment.url}`
              );
    ...
    return {
      ...
      fallbackText:
        syncFallbackText ||
        '**MEDIA PROCESSING**: Analyzing remaining attachments in background...',
      audioTranscription: '',
    };
  }
  ...
  let matchResult = tenorRegex.exec(message.content);
  while (matchResult !== null) {
    hasMedia = true;
    fallbackText += `**GIF MEDIA**: Tenor animated GIF from ${matchResult[0]} `;
    // Note: Full processing would require Tenor API key to fetch GIF data
    matchResult = tenorRegex.exec(message.content);
  }
      ...
      try {
        if (attachment.contentType.startsWith('image/')) {
          ...
          try {
            ...
            logger.error(
              `Video processing failed for ${attachment.url}:`,
              videoError
            );
          ...
          try {
            ...
            logger.error(
              `GIF processing failed for ${attachment.url}:`,
              gifError
            );
            // Fallback to text description
          ...
          try {
            ...
            logger.error(
              `Audio processing failed for ${attachment.url}:`,
              audioError
            );
            // Fallback to text description
      ...
      try {
        ...
        if (stickerUrl && sticker.format !== 'LOTTIE') {
          ...
          if (imageData) {
            ...
            logger.debug(
              `Failed to get image data for sticker: ${sticker.name}`
            );
          }
        } else {
          ...
          fallbackText += fallback + ' ';
          logger.debug(
            `Using fallback for sticker: ${sticker.name} (no URL or LOTTIE format)`
          );
        }
      } catch (error) {
        logger.error(`Failed to download sticker ${sticker.name}:`, error);
  ...
  if (multimodalContent.length > 32) {
    logger.warn(
      `Too many images (${multimodalContent.length}), truncating to prevent API errors and context overload`
    );
  ...
  return {
    ...
    audioTranscription: audioTranscription.trim(),
  };
}
