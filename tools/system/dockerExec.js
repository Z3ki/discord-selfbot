...
export const dockerExecTool = {
  ...
  parameters: {
    ...
    properties: {
      command: {
        type: 'string',
        description: 'The command to execute within the Docker container',
      },
      timeout: {
        type: 'number',
        description:
          'REQUIRED: Timeout in seconds based on command type (5-10 for quick, 15-30 for network, 30-60 for downloads/installs)',
      },
    },
...
export async function executeShell(args, progressCallback = null) {
  ...
  try {
    ...
    const execProcess = spawn(
      'docker',
      ['exec', containerName, 'bash', '-c', envCommand],
      {
        stdio: ['pipe', 'pipe', 'pipe'],
      }
    );
    ...
    logger.info('Docker command executed', {
      command,
      exitCode,
    ...
    if (exitCode === 0) {
      ...
      if (
        stderr.toLowerCase().includes('json') ||
        stderr.toLowerCase().includes('parse')
      ) {
        formattedOutput +=
          'NOTE: This is a server-side JSON parsing error, NOT a formatting issue with your command. The command executed but the server could not parse the response.\n';
      }
      if (
        stderr.toLowerCase().includes('connection') ||
        stderr.toLowerCase().includes('network') ||
        stderr.toLowerCase().includes('refused')
      ) {
        formattedOutput +=
          'NOTE: This is a network connectivity error, NOT a formatting issue. The server is unreachable or the port is closed.\n';
      }
      if (
        ...
        formattedOutput +=
          'NOTE: This is an authentication error, NOT a formatting issue. Your credentials are invalid or missing.\n';
      }
      if (
        ...
        formattedOutput +=
          'NOTE: This is a "not found" error, NOT a formatting issue. The endpoint or resource does not exist.\n';
      }
      if (exitCode === 127) {
        formattedOutput +=
          'NOTE: Command not found error. The executable does not exist in the container.\n';
      }
    }
    ...
    if (stdout.trim()) {
      ...
      if (stdout.length > 1500) {
        ...
        formattedOutput +=
          '... (showing last ' + tailLength + ' characters of output)\n';
