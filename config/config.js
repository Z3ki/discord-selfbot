...
export function validateConfig() {
  ...
  const discordTokenRequired =
    !process.env.DISCORD_TOKEN && !process.env.DISCORD_USER_TOKEN;
  ...
  optional.forEach((key) => {
    if (process.env[key]) {
      ...
      logger.warn(
        `Optional API key missing: ${key} - fallback provider may not work`
      );
    }
  });
  ...
  if (
    isNaN(CONFIG.ai.nvidia.temperature) ||
    CONFIG.ai.nvidia.temperature < 0 ||
    CONFIG.ai.nvidia.temperature > 2
  ) {
