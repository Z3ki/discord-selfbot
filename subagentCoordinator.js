...
export class SubagentCoordinator {
  ...
  registerSubagent(agentId, capabilities, config = {}) {
    ...
    this.apiResourceManager.setAgentQuota(agentId, subagent.config.requestsPerMinute, subagent.config.requestsPerHour);

    logger.info('Subagent registered', { agentId, capabilities, taskRouterSize: this.taskRouter.size });
  }

  async delegateTask(taskType, taskData, priority = 'NORMAL') {
    logger.debug('Delegating task', { taskType, availableAgents: this.taskRouter.get(taskType) });
    const availableAgents = this.taskRouter.get(taskType) || [];

    for (const agentId of availableAgents) {
      const agent = this.subagents.get(agentId);
      if (agent.status === 'idle' && this.apiResourceManager.canMakeRequest(agentId)) {
        agent.status = 'busy';
        try {
          const result = await this.requestQueue.addSubagentTask(agentId,
            () => this.executeSubagentTask(agentId, taskData), priority);
          return result;
        } finally {
          agent.status = 'idle';
        }
...
Provide a detailed, helpful response:`;
    ...
    if (responseText && typeof responseText === 'string' && responseText.length > maxLength) {
